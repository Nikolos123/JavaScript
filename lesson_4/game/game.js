...
const game = {
  ...
  start() {
    ...
    while (+nextQuestionId !== -1) {
      ...
      // debugger
      // let c = this.textNodes;
      // let d = Number(nextQuestionId);
      // let a = this.findArray(this.textNodes, nextQuestionId)
      const nextQuestion = this.findArray(this.textNodes, nextQuestionId);
      // const nextQuestion = this.textNodes.find((q) => q.id === +nextQuestionId);
      nextQuestionId = this.makeStep(nextQuestion);
    }
  },

  findArray(obj, nextQuestionId) {
    for (let i = 0; i < obj.length; i++) {
      if (obj[i].id = nextQuestionId) {
        return obj[i]
      }
